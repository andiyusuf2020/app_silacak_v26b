...
use App\Models\ExcelRealRupModel;
use PhpOffice\PhpSpreadsheet\IOFactory;
use PhpOffice\PhpSpreadsheet\Shared\Date;

class ExcelUploadRealRup extends BaseController
{
    protected $model;
    ...
    public function __construct()
    {
        ...
        $this->model = new ExcelRealRupModel();
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'groupuser' => $namagroup,
            'groupmenu' => $namagroup,
            'title' => 'Upload Data Realisasi PBJ berdasarkan Rencana Umum Pengadaan (RUP)',
            'status' => 'realrup',
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                $dataToInsert[] = [
                    ...
                    'Jenis_Pengadaan' => $this->security->sanitizeFilename($row[3] ?? ''),
                    'Kode_Paket' => $this->security->sanitizeFilename($row[4] ?? ''),
                    ...
                    'Nama_Penyedia' => $this->security->sanitizeFilename($row[9] ?? ''),
                    'Nama_Satuan_Kerja' => $this->security->sanitizeFilename($row[10] ?? ''),
                    'Nilai_PDN' => $this->security->sanitizeFilename($row[11] ?? ''),
                    'Status_Paket' => $this->security->sanitizeFilename($row[12] ?? ''),
                    'Sumber_Dana' => $this->security->sanitizeFilename($row[13] ?? ''),
                    'Sumber_Transaksi' => $this->security->sanitizeFilename($row[14] ?? ''),
                    'Tahun_Anggaran' => $this->security->sanitizeFilename($row[15] ?? ''),
                    'Total_Nilai',
                ];
            }
            ...
            return redirect()->to('adminprov/uploadrealrup')->with('success', 'Data berhasil diupload: ' . count($dataToInsert) . ' record');
        } catch (\Exception $e) {
