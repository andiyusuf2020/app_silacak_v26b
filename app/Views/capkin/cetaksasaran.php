...
<body>
    ...
    ?>
    <h2 style="font-size:12px; text-align:center;">LAPORAN KINERJA PROGRAM PRIORITAS PENDUKUNG SASARAN PEMBANGUNAN TA
        <?= esc($tahun) ?> Bulan <?= esc($bulantw) ?> <br><?= esc($datauser['sub_unit']) ?></h2><br>
    <table cellpadding="4" style="text-align:center">
        ...
        <tbody>
            <tr>
                ...
                <th style="font-size:9px; text-align:center; width:10%" rowspan="2"><strong>SASARAN PROGRAM PRIORITAS PEMBANGUNAN YANG DIDUKUNG</strong></th>
                        ...
                        foreach ($dataDRKP as $key => $value) { ?>
                            <tr>
                                ...
                                <td style="border: 1px solid #000000; font-size:9px; text-align:left; width:26%;">
                                    <br>Uraian Realisasai AKtifitas/Keg.Pokok:
                                    <br><strong><?= esc($rkegpokok['r_uraian']) ?></strong>

                                    <br><img src="uploads/dokumentasi/<?= esc($value['gambar'] ?? null) ?>" width="100"><br>
                                    <?= esc($value['deskripsi']) ?>
                                    Lokasi Aktifitas:
    ...
    <table>
        ...
        <tr>
            <td colspan="2">
                <strong> Anggaran <?= esc($datauser['sub_unit']) ?> s.d. 30 <?= esc($bulantw) ?> 2025
                    adalah sebesar Rp <?= esc(number_format($datarealisasi['realisasi'] ?? null, 0, ',', '.')) ?> atau …% dari total pagu DPA APBD TA 2025 sebesar Rp <?= esc(number_format($jmltotpagu['pagu_rincian'], 0, ',', '.')) ?></strong>
            </td>
        </tr>
        ...
        <tr>
            <br>
            <td style="text-align:right; width:4%;">1</td>
            <td style="text-align:left; width:96%;">
                <br><?= esc($datauser['sub_unit']) ?> mendukung Sasaran Program Prioritas
                <?php
                foreach ($progprio as $key => $dataprogprio) {
                    echo esc($dataprogprio['nm_progprioritas'] . ',');
                } ?>
                <!-- mendukung Program Unggulan XX dan mendukung Program Tematik Pembangunan XX, -->
                dalam mendukung program-program Pembangunan Provinsi Lampung tersebut
                <?= esc($datauser['sub_unit']) ?> telah memapping sebanyak
                <?php
                // echo dd($tjsubkeg);
                // $jSK = count($tjsubkeg) ?? null;

                // echo $jSK;
                ?>
                Subkegiatan yang diurai kembali kedalam
                bentuk aktifitas kegiatan/kegiatan pokok sebanyak
                <?php
                // $jA = count($tjkegpokok) ?? null;
                // echo $jA;
                ?>
                aktifitas kegiatan.
                Dukungan terhadap program-program Pembangunan Daerah termapping pada tiap-tiap
                aktifitas kegiatan sehingga bentuk dukungan
                <?= esc($datauser['sub_unit']) ?> dapat tergambarkan
                lebih transparan dan terukur seperti yang telah dijelaskan pada tabel data diatas.
            </td>
        </tr>
        <tr>
            <br>
            <td style="text-align:right; width:4%;">2</td>
            <td style="text-align:left; width:96%;">
                <br>Pelaksanaan Aktifitas Kegiatan/Kegiatan Pokok dalam Program Kegiatan
                <?= esc($datauser['sub_unit']) ?> sampai Bulan <?= esc($bulantw) ?> telah terealisasi sebanyak
                <?php
                // $jRA = count($rkegpokok);
                // echo $jRA;
                ?>
                aktifitas
                kegiatan/kegiatan pokok atau sebesar XX % dari total YY aktifitas/kegiatan pokok yang direncanakan.
                Aktifitas Kegiatan yang dilaksanakan Dinas XX didukung oleh pagu anggaran pada
                TA xx sebesar Rp …xx yang terbagi dalam Subkegiatan masing-masing dengan telah terealisasi sampai saat
                ini sebesar Rp ..xx… atau sebesar xx. % dari total anggaran.
            </td>
        </tr>
    </table>
    <br>
