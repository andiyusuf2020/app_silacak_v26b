...
<body>
    <div class="container">
        ...
        <div class="action-buttons">
            <a href="/lokasi/tambah" class="btn btn-primary"><i class="bi bi-plus-circle"></i> Tambah Lokasi</a>

            <form action="/lokasi" method="get" class="search-form">
                ...
                <div class="lokasi-list">
                    ...
                    <?php else: ?>
                        <?php foreach ($lokasi as $item): ?>
                            <?php if ($item['gambar']): ?>
                                <img src="/uploads/lokasi/<?= esc($item['gambar']) ?>"
                            ...
                            <div class="lokasi-item" data-lat="<?= $item['latitude'] ?>" data-lng="<?= $item['longitude'] ?>">
                                ...
                                <div class="action-links">
                                    <a href="/lokasi/detail/<?= $item['id'] ?>" class="btn btn-sm btn-info"><i class="bi bi-eye"></i></a>
                                    <a href="/lokasi/edit/<?= $item['id'] ?>" class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i></a>
                                    <form action="/lokasi/hapus/<?= $item['id'] ?>" method="post" class="d-inline">
                                        <?= csrf_field() ?>
    ...
    <script>
        ...
        function createMarkers(lokasiData) {
            ...
            lokasiData.forEach(lokasi => {
                ...
                <img src="/uploads/lokasi/popup/${lokasi.gambar_popup}" 
                     class="popup-image" 
                     alt="${lokasi.nama}">
            </div>`;
                } else if (lokasi.gambar) {
                    popupContent += `<div class="popup-image-container">
                <img src="/uploads/lokasi/${lokasi.gambar}" 
                ...
                const customIcon = lokasi.gambar ? L.icon({
                    iconUrl: `/uploads/lokasi/${lokasi.gambar}`,
                    iconSize: [40, 40],
                    iconAnchor: [20, 40],
                    popupAnchor: [0, -40],
