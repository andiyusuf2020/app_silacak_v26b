        ...
        <div class="card">
            ...
            ?>
                    ...
                    <table class="table table-head-fixed text-wrap">
                        <thead>
                            <tr>
                                ...
                                <th>Pagu</th>
                                <?php if ($bulanpilih == '') { ?>
                                    <th>Total Realisasi s/d Bulan <?= esc($jadwalaktif['bulan']) ?> </th>
                                <?php } else { ?>
                                    <th>Total Realisasi <?= esc($bulanpilih) ?>
                                    </th>
                                <?php  }  ?>
                                <th>% Realisasi</th>
                                <?php if ($bulanpilih == '') { ?>
                ...
                ?>
                    <tr>
                        <td></td>
                        <td colspan="4"><?= esc($rowprogram['nm_urusan']) ?></td>
                    </tr>
                    <tr>
                        <td></td>
                                        ...
                                        */
                                        ...
                                        if ($cektotal == null) {
                                            ...
                                            $dataTR = [
                                                ...
                                                'nm_subkegiatan' => $rowsubkegiatan['nm_subkegiatan'],
                                                'pagu_subkeg' => $rowsubkegiatan['pagu_rincian'],
                                                'total_realisasi' => $jumlahRperSK ?? null,
                                            ];
                                            //  echo dd($dataTR);
                                            // echo dd("update");
                                            $this->totalrealisasiM->save($dataTR);
                                ...
                                <?php
                                    if ($bulanpilih == '') {
                                        $datarincireal2 = $this->realisasilrfkrinci
                                            ->select('*')
                                            ->selectSum('realisasi')
                                            //->select('(sum(realisasi) / pagu_rincian)*100 as isi')
                                            // ->select('((format((realisasi / pagu_rincian),2))) as isi2')
                                            ->where('kd_sub_unit', $value['kd_sub_unit'])
                                            ->where('tahun', $data['tahun'])
                                            ->where('bulan',  $jadwalaktif['bulan'])
                                            ->where('kd_subkegiatan', $rowsubkegiatan['kd_subkegiatan'])
                                            ->groupBy('kd_rekbelanja')
                                        ...
                                        $datarincireal2 = $this->realisasilrfkrinci
                                            ->select('*')
                                            ->selectSum('realisasi')
                                            //->select('(realisasi / pagu_rincian)*100 as isi')
                                            // ->select('((format((realisasi / pagu_rincian),2))) as isi2')
                                            ->where('kd_sub_unit', $value['kd_sub_unit'])
                                            ->where('tahun', $data['tahun'])
                                            ->where('bulan',  $bulanpilih)
...
<tr>
    ...
    if ($bulanpilih == '') {
        ...
        // $jmltotpagu = array_sum($value['pagu_rincian']);
        //echo dd($totalReal);
        if ($totalReal == Null) {
    ?>
            <td colspan="2">Jumlah Total</td>
            <td><?= 'Rp ' . esc(number_format($jmltotpagu['pagu_rincian'], 2, ',', '.')) ?></td>
    ...
    if ($bulanpilih == '') {
        ...
        if (count($da) == 0) {
            ...
            $croopd = (pow((float)$mul, 1 / count($da)) - 1) * 100; ?>
            <td><?= esc(number_format($croopd, 2, ".", ",")); ?></td>
